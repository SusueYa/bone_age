...
class WeightedKappaLoss(Module):
    ...
    def kappa_loss(self, y_pred, y_true):
        ...
        # print(f"O: {O.item()}, E: {E.item()}, Loss: {O / (E + self.epsilon)}")  # ✅ 确保 O 和 E 计算合理

        return O / (E + self.epsilon)

    def forward(self, y_pred, y_true, log=False):
        # if not self.regression:
        #     y_pred = self.softmax(y_pred)
        y_true = y_true.long()
        
        loss = self.kappa_loss(y_pred, y_true)
        # print(f"WeightedKappaLoss raw loss: {loss.item()}") 
