...
class CLM(Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.thresholds_b_ = torch.nn.Parameter(
            data=torch.Tensor([-0.5]), requires_grad=True
        )

        # self.thresholds_a_ = torch.nn.Parameter(
        #     torch.linspace(0.1, 2.0, steps=self.num_classes - 2), requires_grad=True
        # )

        quantiles = torch.tensor([0.2, 0.4, 0.6, 0.8])  # 你可以调整这个量化间隔
        self.thresholds_a_ = torch.nn.Parameter(torch.quantile(predicted_scores, quantiles).detach(), requires_grad=True)
        ...
        # self.thresholds_a_ = torch.nn.Parameter(
        #     torch.randn(self.num_classes - 2) * 0.5, requires_grad=True
        # )

        # 添加用于存储 min/max 的变量
        self.last_min_prob = None
    ...
    def _compute_z3(self, projected: torch.Tensor, thresholds: torch.Tensor):
        ...
        # print(f"z3 range before clipping: min={z3.min().item()}, max={z3.max().item()}")
    ...
    def _apply_link_function(self, z3):
        ...
        # 记录 min/max
        self.last_min_prob = a3T.min().item()
        self.last_max_prob = a3T.max().item()

        # print(f"CLM output range: min={a3T.min().item()}, max={a3T.max().item()}")
